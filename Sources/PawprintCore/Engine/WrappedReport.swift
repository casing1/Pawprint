...
package struct WrappedReport {
    package struct Slide: Identifiable {
        package enum Kind {
            case opener
        ...
        package var footnote: String?
    }

    package var monthKey: String          // "2026-07"
    package var title: String             // "2026년 7월"
    package var dayCount: Int
    package var slides: [Slide]
    /// Headline figures reused by the shareable summary card at the end.
    package var summaryItems: [(String, String)]
    ...
    static package func monthTitle(for monthKey: String) -> String {
    ...
    static package func build(monthKey: String, summaries: [DailySummary], previousMonth: [DailySummary]) -> WrappedReport? {
    ...
    static package func availableMonths(from summaries: [DailySummary]) -> [String] {
...
package struct WrappedMonth: Identifiable {
    package var id: String

    /// Month key immediately preceding the given one, used for the month-over-month slide.
    static package func previousKey(of monthKey: String) -> String {
        let parts = monthKey.split(separator: "-")
        guard parts.count == 2, let year = Int(parts[0]), let month = Int(parts[1]) else { return monthKey }
        return month == 1
