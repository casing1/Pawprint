...
package enum SQLiteError: Error, LocalizedError {
...
package protocol SQLiteBindable {
...
extension String: SQLiteBindable {
    package func bind(to stmt: OpaquePointer?, index: Int32) {
...
final package class SQLiteDatabase {
    ...
    @discardableResult
    package func run(_ sql: String, _ bindings: [SQLiteBindable] = []) throws -> Int {
        try queue.sync {
    ...
    package func query(_ sql: String, _ bindings: [SQLiteBindable] = [], _ rowHandler: (OpaquePointer) -> Void) throws {
        try queue.sync {
            let stmt = try prepareLocked(sql)
            defer { sqlite3_finalize(stmt) }
            for (i, b) in bindings.enumerated() {
...
extension OpaquePointer {
    package func columnText(_ index: Int32) -> String? {
        guard let cString = sqlite3_column_text(self, index) else { return nil }
        return String(cString: cString)
    }

    package func columnInt(_ index: Int32) -> Int {
