...
final package class BatteryHardware {
    static package let shared = BatteryHardware()

    /// Energy stored in a full charge, in watt-hours. Nil on desktop Macs (no battery).
    package private(set) var fullChargeWattHours: Double?
    /// Battery charge cycles the pack has been through.
    package private(set) var cycleCount: Int?
    /// Battery health as a percentage of its original design capacity.
    package private(set) var healthPercent: Int?
