...
package enum MenuBarIconStyle: String, Codable, CaseIterable {
    case paw
    case cat
...
package enum MenuBarMetric: String, Codable, CaseIterable {
    case iconOnly
    case wpm
    case totalKeys
    case focusTime
...
package enum AppTheme: String, Codable, CaseIterable {
...
package struct ExcludedApp: Codable, Identifiable, Hashable {
    package var id: String { bundleID }
    package var bundleID: String
    package var displayName: String
    package var isDefault: Bool = false
...
package struct AppSettings: Codable {
    package var launchAtLogin: Bool = false
    package var showDockIcon: Bool = false
    package var menuBarMetric: MenuBarMetric = .wpm
    /// The paw stays the default — it is what the app has always looked like in the menu bar.
    package var menuBarIcon: MenuBarIconStyle = .paw
    package var dayStartHour: Int = 0
    package var theme: AppTheme = .system
    /// `.system` follows the Mac's preferred languages, falling back to Korean when none of them
    /// has a pack.
    package var language: AppLanguage = .system

    package var collectKeyboard: Bool = true
    package var collectMouse: Bool = true
    package var collectAppUsage: Bool = true
    package var collectClipboard: Bool = true
    package var collectSleepWake: Bool = true
    package var collectPowerPeripherals: Bool = true

    package var isPaused: Bool = false

    package var excludedApps: [ExcludedApp] = AppSettings.defaultExcludedApps
    ...
    package var focusThresholdSeconds: Int = AppSettings.focusThresholdDefault
    static package let focusThresholdDefault = 5 * 60
    /// Days of history to keep. 0 means keep forever.
    package var retentionDays: Int = 90

    /// Headline cards on the Today tab, stored as `MetricCatalog` ids.
    package var dashboardCardIDs: [String] = MetricCatalog.defaultCardIDs
    /// Which metric shades the activity calendar, as a `MetricCatalog` id.
    package var calendarMetricID: String = MetricCatalog.defaultCalendarID
    /// Metrics shown on the shareable card, as `MetricCatalog` ids.
    package var shareCardMetricIDs: [String] = MetricCatalog.defaultShareIDs

    /// Opt-in daily recap notification. Off by default — the app should never nag.
    package var dailySummaryEnabled: Bool = false
    package var dailySummaryHour: Int = 21
    package var dailySummaryMinute: Int = 0
    /// Quiet celebration when a level track advances or a personal record falls.
    package var celebrationNotificationsEnabled: Bool = false
    ...
    package var notifiedQuestLevels: [String: Int] = [:]
    ...
    package var celebratedRecords: [String] = []
    ...
    package var recordDays: [String] = []
    /// Day and running count for the per-day cap.
    package var notificationDay: String = ""
    package var notificationCountToday: Int = 0
    /// Even a genuinely productive day shouldn't produce a stream of alerts.
    static package let maxAchievementNotificationsPerDay = 2

    // MARK: Live HUD
    package var hudCompact: Bool = false
    /// 0.35...1.0. Below ~0.35 the text stops being legible.
    package var hudOpacity: Double = 1.0
    /// Today-metrics shown under the gauge, as `MetricCatalog` ids.
    package var hudMetricIDs: [String] = ["totalKeys", "focusTime"]
    /// Session rows are separate from the catalog — they describe the current stretch of work,
    /// which has no all-day equivalent.
    package var hudShowsSessionTime: Bool = true
    package var hudShowsSessionKeys: Bool = true
    package var hudShowsSessionClicks: Bool = true
    ...
    package var updateCheckEnabled: Bool = true
    ...
    package var updateFeedURL: String = UpdateDistribution.feedURL
    /// Skip the daily background check but keep the manual button working.
    package var updateCheckAutomatically: Bool = true
    ...
    package var dismissedAnnouncements: [String] = []
    ...
    package var updateDefaultsMigrated: Bool = true
    ...
    package func migratedForUpdateDefaults() -> AppSettings? {
    ...
    static package let maxHUDMetrics = 4

    static package let maxDashboardCards = 6
    /// The share card lays out a 4-wide grid; 8 keeps it to two tidy rows.
    static package let maxShareCardMetrics = 8

    package init() {}

    package func isCollecting(_ category: CollectionCategory) -> Bool {
        switch category {
    ...
    static package var defaultExcludedApps: [ExcludedApp] { [
